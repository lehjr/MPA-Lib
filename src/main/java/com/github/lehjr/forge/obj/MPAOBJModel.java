...
public class MPAOBJModel implements IMultipartModelGeometry<MPAOBJModel> {
    ...
    MPAOBJModel(LineReader reader, ModelSettings settings) throws IOException {
    ...
    public static Vector3f parseVector4To3(String[] line) {
        switch (line.length) {
            ...
            case 2:
                return new Vector3f(Float.parseFloat(line[1]), 0, 0);
            case 3:
                return new Vector3f(Float.parseFloat(line[1]), Float.parseFloat(line[2]), 0);
            case 4:
    ...
    public static Vector4f parseVector4(String[] line) {
        switch (line.length) {
            ...
            case 2:
                return new Vector4f(Float.parseFloat(line[1]), 0, 0, 1);
            case 3:
                return new Vector4f(Float.parseFloat(line[1]), Float.parseFloat(line[2]), 0, 1);
            case 4:
                return new Vector4f(Float.parseFloat(line[1]), Float.parseFloat(line[2]), Float.parseFloat(line[3]), 1);
            default:
    ...
    private org.apache.commons.lang3.tuple.Pair<BakedQuad, Direction> makeQuad(int[][] indices, int tintIndex, Vector4f colorTint, net.minecraft.client.renderer.Vector4f
            ambientColor, TextureAtlasSprite texture, TransformationMatrix transform) {
        ...
        return org.apache.commons.lang3.tuple.Pair.of(builder.build(), cull);
    }

    private void putVertexData(IVertexConsumer consumer, net.minecraft.client.renderer.Vector4f position0, Vec2f texCoord0, Vector3f normal0, net.minecraft.client.renderer.Vector4f
            color0, Vec2f uv2, TextureAtlasSprite texture) {
        ...
        public void addQuads(IModelConfiguration owner, IModelBuilder<?> modelBuilder, ModelBakery bakery, Function<Material, TextureAtlasSprite> spriteGetter, IModelTransform modelTransform, ResourceLocation modelLocation) {
            for (ModelMesh mesh : meshes) {
                ...
                for (int[][] face : mesh.faces) {
                    org.apache.commons.lang3.tuple.Pair<BakedQuad, Direction> quad = makeQuad(face, tintIndex, colorTint, mat.ambientColor, texture, modelTransform.getRotation());
