...
public class MPAArmorStandEntity extends LivingEntity {
    ...
    public static final DataParameter<Rotations> LEFT_ARM_ROTATION = EntityDataManager.createKey(MPAArmorStandEntity.class, DataSerializers.ROTATIONS);
        ...
        if (!this.hasMarker() && itemstack.getItem() != Items.NAME_TAG) {
            if (player.isSpectator()) {
                ...
                if (itemstack.isEmpty()) {
                    ...
                    if (this.hasItemInSlot(equipmentslottype2) && this.func_226529_a_(player, equipmentslottype2, itemstack, hand)) {
                        return ActionResultType.SUCCESS;
                    }
                } else {
                    ...
                    if (this.func_226529_a_(player, equipmentslottype, itemstack, hand)) {
                        return ActionResultType.SUCCESS;
                    }
                }
    ...
    private boolean func_226529_a_(PlayerEntity playerEntity, EquipmentSlotType slotType, ItemStack p_226529_3_, Hand p_226529_4_) {
        ItemStack itemstack = this.getItemStackFromSlot(slotType);
        if (!itemstack.isEmpty() && (this.disabledSlots & 1 << slotType.getSlotIndex() + 8) != 0) {
            return false;
        } else if (itemstack.isEmpty() && (this.disabledSlots & 1 << slotType.getSlotIndex() + 16) != 0) {
            return false;
        } else if (playerEntity.abilities.isCreativeMode && itemstack.isEmpty() && !p_226529_3_.isEmpty()) {
            ItemStack itemstack2 = p_226529_3_.copy();
            itemstack2.setCount(1);
            this.setItemStackToSlot(slotType, itemstack2);
            return true;
            ...
            if (!itemstack.isEmpty()) {
                ...
                this.setItemStackToSlot(slotType, itemstack1);
                p_226529_3_.shrink(1);
                return true;
            }
        } else {
            this.setItemStackToSlot(slotType, p_226529_3_);
            playerEntity.setHeldItem(p_226529_4_, itemstack);
            return true;
        }
    }
    ...
    private void playParticles() {
        if (this.world instanceof ServerWorld) {
            ((ServerWorld)this.world).spawnParticle(new BlockParticleData(ParticleTypes.BLOCK, Blocks.OAK_PLANKS.getDefaultState()), this.getPosX(), this.getPosYHeight(0.6666666666666666D), this.getPosZ(), 10, this.getWidth() / 4.0F, this.getHeight() / 4.0F, this.getWidth() / 4.0F, 0.05D);
        }
    ...
    private void func_213815_f(DamageSource p_213815_1_) {
        Block.spawnAsEntity(this.world, this.func_233580_cy_(), new ItemStack(ModObjects.ARMOR_STATION_ITEM.get()));
    ...
    private void func_213816_g(DamageSource p_213816_1_) {
        ...
        for(int i = 0; i < this.handItems.size(); ++i) {
            ...
            if (!itemstack.isEmpty()) {
                Block.spawnAsEntity(this.world, this.func_233580_cy_().up(), itemstack);
        ...
        for(int j = 0; j < this.armorItems.size(); ++j) {
            ...
            if (!itemstack1.isEmpty()) {
                Block.spawnAsEntity(this.world, this.func_233580_cy_().up(), itemstack1);
    ...
    public boolean hitByEntity(Entity entityIn) {
        return entityIn instanceof PlayerEntity && !this.world.isBlockModifiable((PlayerEntity)entityIn, this.func_233580_cy_());
    }
