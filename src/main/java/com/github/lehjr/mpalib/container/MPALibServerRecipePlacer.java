...
public class MPALibServerRecipePlacer extends ServerRecipePlacer {
    public MPALibServerRecipePlacer(RecipeBookContainer recipeBookContainer) {
        ...
        if (recipeIn != null && (player.getRecipeBook().isUnlocked(recipeIn) ||
            ...
            if (this.placeIntoInventory() || player.isCreative()) {
                ...
                if (this.recipeItemHelper.canCraft(recipeIn, null)) {
                    ...
                    this.clear();
                    if (this.recipeBookContainer instanceof ArmorStandModdingContainer) {
                        MPALibPackets.CHANNEL_INSTANCE.send(PacketDistributor.PLAYER.with(()-> player),
