...
import com.github.lehjr.mpalib.network.packets.MoveModuleFromSlotToSlotPacket;
...
import static com.github.lehjr.mpalib.basemod.ModObjects.ARMOR_STAND_MODDING_CONTAINER_CONTAINER_TYPE;
 ...
 *
 */
public class ArmorStandModdingContainer extends MPALibRecipeBookContainer<CraftingInventory> implements IModularItemToSlotMapProvider {
    ...
    public ArmorStandModdingContainer(int windowId, PlayerInventory playerInventory) {
        super(ARMOR_STAND_MODDING_CONTAINER_CONTAINER_TYPE.get(), windowId);
        ...
        if(sourceSlot.canTakeStack(player) && canMergeSlot(contents, targetSlot)) {
            if (source == getOutputSlot()) {
                if (this.mergeItemStack(stackCopy, target, target+ 1, false)) {
                    ...
                    MPALibPackets.CHANNEL_INSTANCE.sendToServer(new MoveModuleFromSlotToSlotPacket(this.windowId, source, target));
                }
            } else {
                MPALibPackets.CHANNEL_INSTANCE.sendToServer(new MoveModuleFromSlotToSlotPacket(this.windowId, source, target));
