 ...
 */
public class MPAOBJLoader implements IModelLoader<MPAOBJModel> {
    public static MPAOBJLoader INSTANCE = new MPAOBJLoader();

    private final Map<MPAOBJModel.ModelSettings, MPAOBJModel> modelCache = Maps.newHashMap();
    ...
    @Override
    public MPAOBJModel read(JsonDeserializationContext deserializationContext, JsonObject modelContents) {
        ...
        return loadModel(new MPAOBJModel.ModelSettings(new ResourceLocation(modelLocation), detectCullableFaces, diffuseLighting, flipV, ambientToFullbright, materialLibraryOverrideLocation));
    }

    public MPAOBJModel loadModel(MPAOBJModel.ModelSettings settings) {
        return modelCache.computeIfAbsent(settings, (data) -> {
            ...
            try (LineReader rdr = new LineReader(resource)) {
                return new MPAOBJModel(rdr, settings);
            } catch (Exception e) {
